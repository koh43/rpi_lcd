...
class LCD_2inch(lcdconfig.RaspberryPi):
    ...
    def ShowImageCV(self, img, rot_ang=0, Xstart=0, Ystart=0):
        """Set buffer from OpenCV image"""
        """Write display buffer to physical display"""
        ...
        img = img[:,:,::-1] # OpenCV images are BGR, need to change to RGB
        img = cv2.resize(img, (self.height, self.width)) # fit to window size
        ...
        self.command(0x36)
        self.data(0x70)
        self.SetWindows(Xstart, Ystart, self.height, self.width)
        self.digital_write(self.DC_PIN, True)
        for i in range(0,len(pix),4096):
            self.spi_writebyte(pix[i:i+4096])
